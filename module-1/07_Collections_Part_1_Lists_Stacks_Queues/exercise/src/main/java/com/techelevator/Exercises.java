...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		List<String> string = Arrays.asList(stringArray);
		return string;
	}
	...
	public Integer findLargest(List<Integer> integerList) {
		int Apple = Collections.max(integerList);
		return Apple;
	}
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		for (int index = 0; index < Math.max(listOne.size(), listTwo.size()); index++) {
			if (index < listOne.size()) {
				interLeave.add(listOne.get(index));
			}
			if (index < listTwo.size()) {
				interLeave.add(listTwo.get(index));
			}
		} 
		return interLeave;
	}
